...
# Расчет ресурсов
### Домены `cloud` и `meta` для центрального кластера (Москва)
...
### Общая нагрузка на домен `data`
| Компонент          | Пиковый RPS | Характер сервиса         | CPU         | RAM         | Трафик          |
| ------------------ | ----------- | -----|----------- | ----------- | --------------- |
| **Access_Service** | ~7375        | Средняя бизнес-логика     |74   | ~7.7 GB  | ~103 Мбит/с  |
| **AV_Scanner**     | ~2750        | Тяжелая бизнес-логика     |275 | ~27.6 GB | ~14.4 Гбит/с |
| **S3 Gateway**     | ~6875        | Тяжелая бизнес-логика     |688 | ~69 GB   | ~372 Гбит/с  |
...
| Компонент          | Пиковый RPS | Характер сервиса         | CPU         | RAM          | Трафик           |
| ------------------ | ----------- | -----|----------- | ----------- |------------ |
| **Access_Service** | ~4360        | Средняя бизнес-логика     |44   | ~4.54 GB  | ~60.9 Мбит/с  |
| **AV_Scanner**     | ~1625        | Тяжелая бизнес-логика     |163 | ~16.31 GB | ~8.50 Гбит/с  |
...
### Железо для Москвы (`data` + `meta` + `data`)
| Узел           | Конфигурация                           | Cores | Cnt    | Цена (реальная) |
| -------------- | -------------------------------------- | ----- | ------ | --------------- |
| **kubernode**  | EPYC 7713P / 64GB / 2×NVMe / 2×25GbE   | 64    | **3**  | **€7 200**      |
| **nginx L7**   | EPYC 7313P / 32GB / 1×NVMe / 2×25GbE   | 16    | **1**  | **€4 000**      |
| **nginx L4**   | EPYC 7313P / 32GB / 1×NVMe / 2×100GbE  | 16    | **1**  | **€5 300**      |
| **S3 Gateway** | EPYC 7543P / 128GB / 2×NVMe / 2×100GbE | 32    | **13** | **€11 000**     |
| **AV Scanner** | EPYC 7543P / 64GB / 1×NVMe / 2×25GbE   | 32    | **6**  | **€9 000**      |



### Общая нагрузка на домен `data` в СПБ (`k = 5/22`)
| Компонент              | Пиковый RPS | Характер сервиса      | CPU       | RAM           | Трафик           |
| ---------------------- | ----------- | ----------------- | --------- | ------------- | ---------------- |
| **Access_Service**     | ~1676   | Средняя бизнес-логика | 17    | ~1.75 GB  | ~23.4 Мбит/с |
| **AV_Scanner**         | ~625    | Тяжелая бизнес-логика | 63  | ~6.27 GB  | ~3.27 Гбит/с |
| **S3 Gateway**         | ~1563   | Тяжелая бизнес-логика | 157 | ~15.7 GB  | ~84.5 Гбит/с |
| **NGINX (L7: data)**   | ~1676   | SSL Term + Proxy      | 5  | ~0.043 GB | ~23.4 Мбит/с |
| **NGINX (L4, stream)** | ~1563   | L4 passthrough        | 5  | ~0.041 GB | ~84.5 Гбит/с |
