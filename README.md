...
| **File_Meta_Service** | Управление метаданными и каталогами: создание директорий, листинг, изменение видимости, удаление файлов. |
| **Access_Service** | Управление правами доступа. Обеспечивает быстрый поиск и проверку прав при скачивании. |
| **S3 Gateway** | Промежуточный слой между клиентом и объектным хранилищем **Ceph**. Выполняет:<br>• Кэширование часто запрашиваемых объектов,<br>• вычисление **Whole-file hash** для дедупликации<br>• формирование события `file_uploaded` → **Kafka** (`FileID`, `UserID`, `Url`, `Hash`) для обновления метаданных. |
| **AV_Scanner** | Проверяет загруженные файлы на вирусы (**ClamAV**). Получает события загрузки из Kafka, сканирует содержимое и обновляет статус файла в `FileMetadata`. |
| **Metrics_Service** | Обрабатывает поток событий из Kafka и записывает агрегированные данные в `ClickHouse` для мониторинга и аналитики. |
