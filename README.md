...
| **`data.cloud.mail.ru`** | Операции с файлами | Загрузка, Скачивание, Удаление файлов |

### Расположение ДЦ
Согласно тому, что сервисом в основном пользуются граждане Росси, Беларуси и Казахстана, 
то ДЦ можно расположить внутри РФ (тогда обойдёмся без DNS-балансировки). 
...
| Домен | Города |
| :--- | :--- |
| **`cloud.mail.ru`** | Москва, СПб |
| **`meta.cloud.mail.ru`** | Москва, СПб |
| **`data.cloud.mail.ru`** (S3 + CDN)| Москва, СПб, Краснодар, Казань, Новосибирск, Хабаровровск |

### Схема балансировки
- Все ДЦ внутри своей группы обрабатываемых доменов анонсируют один IP => благодаря BGP Anycast запросы приходят на ближайший ДЦ
