...
| **FileAccess** | `4,550,000,000 записей × 80 bytes`<br>Состав: ID(8) + UserId(8) + Mode(1) + Url(200) + CreatedAt/UpdatedAt(16)<br>Расчет записей: `45,500,000,000 × 0,1` | **360 ГБ** | **4,550,000,000** | **200 QPS**<br>(изменение видимости) | **1650 QPS**<br>(скачивание файлов) |
...
|--------|----------------------|------------------|
| **Сервис загрузки данных → FileMetadata** | `fileId`, `baseUrl` | Асинхронная передача информации о загруженном файле.|
...
|----------|-------------------|---------------------------------|
| **Content-Based Chunking (CDC)** | Дедупликация файлов, оптимизация хранения | Позволяет находить повторяющиеся блоки даже при сдвигах или изменениях внутри файла. Снижает объём хранения. |
| **Perceptual Hashing (pHash)** | Поиск похожих изображений и видео, удаление дублей | Генерирует “отпечаток восприятия” (не побитовый хеш). Позволяет находить визуально схожий контент, даже если он сжат, повернут или отредактирован. |
| **Hash-based malware detection** (Signature DB + Bloom Filter + Rolling Hash) | Проверка файлов на известные угрозы | Быстрое выявление вирусов по сигнатурам без полного сканирования. Bloom Filter обеспечивает быструю проверку по образцам. |
| **Machine Learning for Malware Detection** | Обнаружение модифицированных вирусов | ML-модели выявляют вредоносное поведение по структуре и метаданным файла. |
