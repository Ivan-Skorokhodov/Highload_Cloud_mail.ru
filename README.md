...
# Расчет ресурсов
| Компонент           | Пиковый RPS | Характер сервиса         | CPU | RAM (МБ) | Трафик |
|---------------------|-------------|---------------------------|-----|----------|--------|
| **User_Service**     | 125         | Лёгкое JSON API           | 1   | 10       | ~1 Мбит/с |
| **File_Meta_Service**| 4438        | Средняя бизнес-логика     | 45  | 4500     | ~5 Мбит/с |
| **Access_Service**   | 6875        | Средняя бизнес-логика     | 69  | 6900     | ~10 Мбит/с |
| **S3 Gateway**       | 6875        | Тяжёлая бизнес-логика     | 688 | 68800    | ~370 Гбит/с |
| **AV_Scanner**       | 1100        | Тяжёлая бизнес-логика     | 110 | 11000    | ~5 Гбит/с |
| **Metrics_Service**  | 7925        | Средняя бизнес-логика     | 80  | 8000     | ~2 Мбит/с |
| **Nginx (L7)**       | 8689        | SSL Term + Proxy | 18  | 180      | ~70 Мбит/с |
| **Nginx (L4, stream)** | 6875      | L4 passthrough   | 14  | 140      | ~370 Гбит/с |
### Некоторые пояснения:
**AV_Scanner:**  
Трафик ≈ `RPS × средний размер большого файла` → `1100 × ~50 МБ ≈ 5 Гбит/с`; сервис скачивает крупные файлы из Ceph для антивирусной проверки.

**Metrics_Service:**  
Трафик ≈ `RPS × размер события Kafka` → `7925 × ~300 B ≈ 2 Мбит/с`; сервис получает мелкие события из Kafka и почти не генерирует сетевой нагрузки.

**Nginx (L7):**  
Трафик ≈ `RPS × ~8–10 КБ JSON` → `8689 × ~10 КБ ≈ 70 Мбит/с`; через L7 проходят только небольшие API-запросы без файлового трафика.
